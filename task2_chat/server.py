    ...
    async def distrubute(self, ws: WebSocketServerProtocol):       
        async for message in ws:          
            if 'exchange' in message:
                days = exchange_main.set_days_from_chat_massage(message)
                urls = exchange_main.get_list_of_urls (days)
                currencies = exchange_main.DEFAULT_CURRENCIES
                res = await exchange_main.main(urls, currencies)
                # await self.send_to_clients(r)
                await self.send_to_client(res, ws)
            else:
                await self.send_to_clients(f"Unknown command: {message}")

async def exchange_main():
    server = Server()
    async with websockets.serve(server.ws_handler, 'localhost', 8080):
        await asyncio.Future()  # run forever

if __name__ == '__main__':
    asyncio.run(exchange_main())